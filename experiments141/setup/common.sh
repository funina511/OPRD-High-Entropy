...
# --- interpreter + network: verl py3.12; wandb online via HTTP proxy ---
...
# Only bypass proxy for local traffic. Putting wandb.ai in NO_PROXY forces a
# direct GraphQL path that times out here; keep wandb online and proxy it.
...
# Prefer HTTP(S)_PROXY for wandb-core; socks ALL_PROXY can make init hang.
...
export SKIP_RAY_STOP=${SKIP_RAY_STOP:-0}                   # never kill another run's ray cluster

# --- paths ---
export DATA_DIR=${DATA_DIR:-/root/siton-tmp/home/liuxinyu/OPRD-High-Entropy/datasets}
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-/root/siton-tmp/home/liuxinyu/hf_models/Qwen3-0.6B-Base}
export REWARD_MODEL_PATH=${REWARD_MODEL_PATH:-/root/siton-tmp/home/liuxinyu/hf_models/Qwen3-4B}
