...
# Chunk-level relational alignment loss: mse (default/legacy) | rkd | infonce
export REP_ALIGN_LOSS=${REP_ALIGN_LOSS:-mse}
export REP_CHUNKS=${REP_CHUNKS:-8}
export REP_INFONCE_TAU=${REP_INFONCE_TAU:-0.07}
# RKD-A (within-response) weight vs RKD-D; 0 = distance-only
...
if [ "${SKIP_RAY_STOP:-0}" != "1" ]; then
    ray stop --force || true
    # ray stop often leaves zombies/orphans on this host; sweep leftovers on our port.
...
# Cap object store + CPUs: default prestart workers (== num CPUs) + huge shm was
# triggering intermittent ENOMEM under vm.overcommit_memory=0 during ray.init.
ray start --head --port="$RAY_PORT" --temp-dir="$RAY_TEMP_DIR" \
    --object-store-memory="${RAY_OBJECT_STORE_MEMORY:-10000000000}" \
    --num-cpus="${RAY_NUM_CPUS:-8}"
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.rep_rkd_angle_coef=${REP_RKD_ANGLE_COEF} \
