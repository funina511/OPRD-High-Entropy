...
from verl.utils.memory_utils import aggressive_empty_cache, profile_actor_update_memory
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                with profile_actor_update_memory() as mem_profile_metrics:
                    metrics = self.actor.update_policy(data=data)
            if mem_profile_metrics:
                metrics.update(mem_profile_metrics)
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(
        ...
        with torch.no_grad(), torch.autocast(device_type=get_device_name(), dtype=torch.bfloat16):
            ...
            response_mask = micro_batch.get("response_mask") if return_last_hidden_repr else None
            need_logits = student_top_k_ids is not None or compute_entropy or return_last_hidden_repr
            ...
            if self.use_remove_padding:
                ...
                output = self.reward_module(
                    ...
                    use_cache=False,
                    return_dict=True if return_last_hidden_repr else self.use_fused_kernels,
                    output_hidden_states=return_last_hidden_repr,
                )
                ...
                if return_last_hidden_repr:
                    ...
                    if rep_distillation_layers != "last":
                        teacher_last_hidden_repr = hidden_states_tuple_to_response_repr(
                ...
                if return_last_hidden_repr:
                    from verl.utils.rep_distillation import extract_teacher_response_hidden_repr
            ...
            return (
                ...
                teacher_last_hidden_repr,
            )

    def _extract_teacher_attn_rows(
        self,
        micro_batch,
        *,
        att_distillation_positions: str,
        att_distillation_layers: str,
        att_distillation_last_k: int,
        att_distillation_max_key_len: int,
        att_distillation_first_k: int = 50,
    ):
        from verl.utils.att_distillation import forward_response_attn_rows
        ...
        return forward_response_attn_rows(
            ...
            max_context_len=att_distillation_max_key_len,
        )
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        # Get student log probabilities from trajectories (optional for rep-only distillation)
        student_logp = data.batch.get("old_log_probs")
        ...
        att_distillation_first_k = int(data.meta_info.get("att_distillation_first_k", 50))
        att_distillation_max_key_len = int(data.meta_info.get("att_distillation_max_key_len", 4096))
        # Compute teacher entropy every step for logging, but only plot every 10 steps
        compute_entropy = data.meta_info.get("compute_teacher_entropy", True)
        ...
        with self.ulysses_sharding_manager:
            ...
            dynamic_batch_idx_list = None
            if use_dynamic_bsz:
                max_token_len = self.config.forward_max_token_len_per_gpu * self.ulysses_sequence_parallel_size
                micro_batches, dynamic_batch_idx_list = rearrange_micro_batches(
            ...
            if use_att_distillation:
                from verl.utils.att_distillation import (
                    ...
                    sync_att_distillation_batch_key_width,
                )

                att_batch_key_width = get_att_distillation_batch_key_width(
                    rm_data.batch["input_ids"],
                    rm_data.batch["attention_mask"],
                    rm_data.batch["position_ids"],
                    rm_data.batch["response_mask"],
                    positions=att_distillation_positions,
                    first_k=att_distillation_first_k,
                    last_k=att_distillation_last_k,
                    max_context_len=att_distillation_max_key_len,
                )
                att_batch_key_width = sync_att_distillation_batch_key_width(
                    att_batch_key_width, rm_data.batch["input_ids"].device
                )
            ...
            output_teacher_last_hidden_repr = []
            output_teacher_attn_rows = []
            ...
            for micro_batch in micro_batches:
                ...
                (
                    ...
                    teacher_last_hidden_repr_batch,
                ...
                if use_att_distillation:
                    teacher_attn_rows_batch = self._extract_teacher_attn_rows(
                        ...
                        att_distillation_layers=att_distillation_layers,
                        att_distillation_last_k=att_distillation_last_k,
                        att_distillation_max_key_len=att_distillation_max_key_len,
                        att_distillation_first_k=att_distillation_first_k,
                    )
            ...
            if len(output_teacher_last_hidden_repr) > 0:
                ...
                if len(set(repr_shapes)) > 1:
                    raise RuntimeError(
                        ...
                        f"(likely variable first_k/last_k width): {repr_shapes}"
                    )
                teacher_last_hidden_repr = torch.cat(output_teacher_last_hidden_repr, dim=0)
            ...
            if len(output_teacher_attn_rows) > 0:
                ...
                teacher_attn_rows = torch.cat(output_teacher_attn_rows, dim=0)

            if use_dynamic_bsz:
                from verl.utils.seqlen_balancing import restore_dynamic_batch

                flat_indices = list(itertools.chain.from_iterable(dynamic_batch_idx_list))
                assert len(flat_indices) == teacher_logp.size(0), (
                    f"dynamic batch indices {len(flat_indices)} vs teacher_logp {teacher_logp.size(0)}"
                )
                revert_indices = torch.tensor(
                    get_reverse_idx(flat_indices), dtype=torch.long, device=teacher_logp.device
                )
                ...
                if teacher_last_hidden_repr is not None:
                    expected_repr_batch = len(flat_indices)
                    if teacher_last_hidden_repr.size(0) != expected_repr_batch:
                        raise RuntimeError(
                            "teacher_last_hidden_repr batch size "
                            f"{teacher_last_hidden_repr.size(0)} != expected {expected_repr_batch}; "
                            f"micro-batch repr shapes: "
                            f"{[tuple(t.shape) for t in output_teacher_last_hidden_repr]}"
                        )
                    teacher_last_hidden_repr = restore_dynamic_batch(
                        teacher_last_hidden_repr.contiguous(), dynamic_batch_idx_list
                    )
                if teacher_attn_rows is not None:
                    if teacher_attn_rows.size(0) != len(flat_indices):
                        raise RuntimeError(
                            "teacher_attn_rows batch size "
                            f"{teacher_attn_rows.size(0)} != expected {len(flat_indices)}"
                        )
                    teacher_attn_rows = restore_dynamic_batch(
                        teacher_attn_rows.contiguous(), dynamic_batch_idx_list
                    )
            ...
            if teacher_attn_rows is not None:
                tensors["teacher_attn_rows"] = teacher_attn_rows
