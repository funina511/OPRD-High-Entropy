...
from verl.utils.att_distillation import get_att_distillation_context_metadata
...
def compute_rep_distillation_metrics(batch: DataProto) -> dict[str, Any]:
    """Batch-level diagnostics for representation distillation (driver-side, no extra forward)."""
    if "teacher_last_hidden_repr" not in batch.batch.keys():
        return {}

    teacher_repr = batch.batch["teacher_last_hidden_repr"]
    rep_layers = batch.meta_info.get("rep_distillation_layers", "last")
    rep_positions = batch.meta_info.get("rep_distillation_positions", "last")
    multi_layer_rep = rep_layers != "last"
    if rep_positions in ("first_k", "last_k"):
        from verl.utils.rep_distillation import build_compact_rep_distillation_position_mask

        compact_mask = build_compact_rep_distillation_position_mask(
            batch.batch["response_mask"],
            rep_positions,
            batch.meta_info.get("rep_distillation_last_k", 32),
            batch.meta_info.get("rep_distillation_first_k", 50),
        )
        if teacher_repr.dim() == 4:
            teacher_norm = torch.masked_select(teacher_repr.norm(dim=-1).mean(dim=1), compact_mask.bool())
        else:
            teacher_norm = torch.masked_select(teacher_repr.norm(dim=-1), compact_mask.bool())
    elif teacher_repr.dim() == 4 or (
        teacher_repr.dim() == 3 and multi_layer_rep and rep_positions == "last"
    ):
        teacher_norm = teacher_repr.norm(dim=-1).mean()
    elif teacher_repr.dim() == 3:
        response_mask = batch.batch["response_mask"].bool()
        teacher_norm = torch.masked_select(teacher_repr.norm(dim=-1), response_mask)
    else:
        teacher_norm = teacher_repr.norm(dim=-1)
    metrics: dict[str, Any] = {
        "rep/raw_teacher_repr_norm_mean": teacher_norm.mean().detach().item(),
        "rep/raw_teacher_repr_norm_max": teacher_norm.max().detach().item(),
        "rep/raw_teacher_repr_norm_min": teacher_norm.min().detach().item(),
        "rep/raw_teacher_hidden_dim": teacher_repr.shape[-1],
        "rep/batch_size": float(teacher_repr.shape[0]),
        # Backward-compatible aliases (raw teacher repr, before projector).
        "rep/teacher_repr_norm/mean": teacher_norm.mean().detach().item(),
        "rep/teacher_repr_norm/max": teacher_norm.max().detach().item(),
        "rep/teacher_repr_norm/min": teacher_norm.min().detach().item(),
        "rep/hidden_dim": teacher_repr.shape[-1],
    }
    if rep_positions in ("first_k", "last_k"):
        metrics["rep/compact_token_len"] = float(teacher_repr.size(-2) if teacher_repr.dim() >= 3 else 1)
        metrics["rep/valid_response_tokens_mean"] = float(compact_mask.sum(dim=1).float().mean().detach().item())
        metrics["rep/valid_response_tokens_max"] = float(compact_mask.sum(dim=1).max().detach().item())
    if multi_layer_rep:
        metrics["rep/num_teacher_layers_raw"] = float(teacher_repr.size(1))
        metrics["rep/num_layers"] = teacher_repr.size(1)
    if batch.meta_info.get("rep_distillation_positions") == "last_k":
        metrics["rep/last_k"] = batch.meta_info.get("rep_distillation_last_k", 32)
    if batch.meta_info.get("rep_distillation_positions") == "first_k":
        metrics["rep/first_k"] = batch.meta_info.get("rep_distillation_first_k", 50)
    if "rep_repr_cosine_sim" in batch.batch.keys():
        cosine = batch.batch["rep_repr_cosine_sim"]
        metrics["rep/cosine_similarity/mean"] = cosine.mean().detach().item()
        metrics["rep/cosine_similarity/min"] = cosine.min().detach().item()
        metrics["rep/cosine_similarity/max"] = cosine.max().detach().item()
    return metrics


def compute_att_distillation_metrics(batch: DataProto) -> dict[str, Any]:
    """Batch-level diagnostics for attention distillation."""
    if "teacher_attn_rows" not in batch.batch.keys():
        return {}

    teacher_attn = batch.batch["teacher_attn_rows"]
    metrics: dict[str, Any] = {
        "att/key_len": teacher_attn.shape[-1],
        "att/batch_size": teacher_attn.shape[0],
    }
    att_layers = batch.meta_info.get("att_distillation_layers", "last")
    if att_layers != "last" and teacher_attn.dim() >= 4:
        metrics["att/num_layers"] = teacher_attn.size(1)
    if batch.meta_info.get("att_distillation_positions") == "last_k":
        metrics["att/last_k"] = batch.meta_info.get("att_distillation_last_k", 32)
    if batch.meta_info.get("att_distillation_positions") == "first_k":
        metrics["att/first_k"] = batch.meta_info.get("att_distillation_first_k", 50)
    metrics["att/max_key_len"] = batch.meta_info.get("att_distillation_max_key_len", 4096)
    att_context_keys = ("input_ids", "attention_mask", "position_ids", "response_mask")
    if all(key in batch.batch.keys() for key in att_context_keys):
        _, _, context_len = get_att_distillation_context_metadata(
            batch.batch["input_ids"],
            batch.batch["attention_mask"],
            batch.batch["position_ids"],
            batch.batch["response_mask"],
            positions=batch.meta_info.get("att_distillation_positions", "last"),
            first_k=batch.meta_info.get("att_distillation_first_k", 50),
            last_k=batch.meta_info.get("att_distillation_last_k", 32),
            max_context_len=batch.meta_info.get("att_distillation_max_key_len", 4096),
        )
        metrics["att/context_len"] = context_len
    return metrics


def _compute_sequence_length_metrics(batch: DataProto) -> dict[str, Any]:
    """Length / turn statistics without RL reward or advantage metrics."""
    ...
    metrics = {
        ...
        "response/aborted_ratio": torch.mean(aborted_mask.float()).detach().item(),
...
def compute_data_metrics(
    batch: DataProto, use_critic: bool = True, rep_distillation_only: bool = False
) -> dict[str, Any]:
    """
    ...
    Args:
        ...
        rep_distillation_only: If True, only log sequence-length metrics (no zero RL placeholders).
    ...
    if rep_distillation_only:
        return _compute_sequence_length_metrics(batch)
    ...
    if advantages.dim() == 3:
        ...
        partial_adv_metrics["critic/advantages/mean_sum_over_k"] = torch.mean(valid_adv_sum).detach().item() if valid_adv_sum.numel() > 0 else 0.0
        
        # Method 2: mean over all non-zero values (flattened)
        partial_adv_metrics["critic/advantages/mean_overall"] = torch.mean(valid_adv_nonzero).detach().item() if valid_adv_nonzero.numel() > 0 else 0.0
    else:
        # 2D case: single mean value
        partial_adv_metrics["critic/advantages/mean"] = torch.mean(valid_adv_nonzero).detach().item() if valid_adv_nonzero.numel() > 0 else 0.0
    ...
    if "token_level_advantage_direct" in batch.batch.keys():
        ...
        partial_adv_metrics["critic/advantages/direct_mean"] = torch.mean(valid_direct_adv).detach().item() if valid_direct_adv.numel() > 0 else 0.0
    ...
    metrics = {
        ...
        **partial_adv_metrics,
        ...
        "critic/returns/mean": torch.mean(valid_returns_nonzero).detach().item() if valid_returns_nonzero.numel() > 0 else 0.0,
        **(
            {
                # values
